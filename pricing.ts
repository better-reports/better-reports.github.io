...
class PricingHelper {
    static comutePricingVM(tiers: MeteredPlanTier[]): PricingVM {
        ...
        const tiersVM = tiers.map(t => {
            let tierVM = <TierVM>{
                strRange:
                    (previousTier == null ? 0 : previousTier.maxQtyValue + 1).toLocaleString(
                        'en-US'
                    ) +
    ...
    public static convertSliderValueToQtyValue(sliderValue: number, tiersVM: TierVM[]) {
        ...
        let qtyValue =
            (previousTier == null ? 0 : previousTier.maxQtyValue + tier.sliderStepSize) +
    ...
    public static convertQtyValueToSliderValue(qtyValue: number, tiersVM: TierVM[]) {
        ...
        let sliderValue =
            t.minSliderValue + Math.round((qtyValue - t.minQtyValue) / t.sliderStepSize);
        return sliderValue;
    }
}
//END COPY
...
function renderPricing(connectorName: string, targetEltId) {
    ...
    if (pricing.tiers != null) {
        ...
        if (eltSlider == null)
            // no slider
            return;
        let eltStrQty = <HTMLInputElement>targetElt.querySelector('#tier-cost-qty-value');
        let eltTierCostDetail = <HTMLElement>targetElt.querySelector('#tier-cost-detail');
        eltSlider.oninput = () => {
