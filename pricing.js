...
var PricingHelper = /** @class */ (function () {
  ...
  PricingHelper.comutePricingVM = function (tiers) {
    ...
    var tiersVM = tiers.map(function (t) {
      var tierVM = {
        ...
        strUnitCost:
          t.unitCost == null ? null : "$" + t.unitCost.toLocaleString("en-US"),
    ...
    var hasFlatFees = tiers.some(function (t) {
      return t.flatFee != null;
    });
    var hasUnitCosts = tiers.some(function (t) {
      return t.unitCost != null;
    });
    return {
      tierVMs: tiersVM,
      sliderMax: sliderMax,
  ...
  PricingHelper.computeMeteredPlanCost = function (tiers, actualQty) {
    ...
    for (var _i = 0, tiers_1 = tiers; _i < tiers_1.length; _i++) {
      ...
      var tierVariableCost =
        tierQty * ((_a = tier.unitCost) !== null && _a !== void 0 ? _a : 0);
      var tierTotalCost =
    ...
    var strEstimatedCost =
      tierCosts
        //cannot use ncMapMany from public site, so inline the implementation
        .map(function (tc) {
          return [
            tc.tierUnitCost == null
              ? null
              : "(" +
                tc.tierQuantity.toLocaleString("en-US") +
                " x " +
                tc.tierUnitCost.toLocaleString("en-US") +
                ")",
  ...
  PricingHelper.convertSliderValueToQtyValue = function (sliderValue, tiersVM) {
    ...
    var tier = tiersVM.find(function (t) {
      return t.maxSliderValue >= sliderValue;
    });
  ...
  PricingHelper.convertQtyValueToSliderValue = function (qtyValue, tiersVM) {
    var t = tiersVM.find(function (t) {
      return t.maxQtyValue >= qtyValue;
    });
    var sliderValue =
      t.minSliderValue +
...
connectorToPricing.set("quickbooks", {
  ...
  tierQtyLabelExplanationHtml:
    "Pricing depends on your company's average monthly expenses, calculated with the formula below.<br/>Average monthly expenses = Last quarter total expenses (USD) \u00F7 3",
  unitPriceLabel: "Unit price",
  tiers: [
    { flatFee: 29.9, upperQuantity: 2000 },
    { flatFee: 49.9, upperQuantity: 5000 },
    { flatFee: 99.9, upperQuantity: 10000 },
    { flatFee: 149.9, upperQuantity: 20000 },
    { flatFee: 249.9, upperQuantity: 50000 },
    { flatFee: 399.9, upperQuantity: 100000 },
    { flatFee: 599.9, upperQuantity: 200000 },
    { flatFee: 799.9, upperQuantity: 500000 },
    { flatFee: 999.9, upperQuantity: null },
  ],
...
function renderPricing(connectorName, targetEltId) {
  ...
  if (pricing.tiers != null) {
    ...
    for (var _b = 0, _c = pricingVM.tierVMs; _b < _c.length; _b++) {
      var t = _c[_b];
      html +=
        '<tr class="tiers-data-row">\n                    <td>' +
        t.strRange +
        "</td>\n                    " +
        (!pricingVM.hasUnitCosts ? "" : "<td>" + t.strUnitCost + "</td>") +
        "\n                    " +
        (!pricingVM.hasFlatFees ? "" : "<td>" + t.strFlatFee + "</td>") +
        "\n                </tr>";
    }
    html += "</tbody>";
    html += "</table>";
    ...
    if (!noSlider) {
      ...
      var sliderValue = PricingHelper.convertQtyValueToSliderValue(
        qtyValue,
        pricingVM.tierVMs
      );
      ...
      html +=
        '<div class="tier-cost">\n                  <div class="tier-cost-qty"><span id="tier-cost-qty-label">' +
        pricing.tierQtyLabel +
        '</span>: <span id="tier-cost-qty-value">' +
        strQtyValue +
        '</span></div>\n                  <div class="tier-cost-slide-container">\n                  <input id="slider" type="range" min="0" max="' +
        pricingVM.sliderMax +
        '" value="' +
        sliderValue +
        '">\n                  </div>\n                  <div class="tier-cost-estimated-cost">Estimated cost:</div>\n                  <div id="tier-cost-detail">' +
        cost.strEstimatedCost +
        "</div>\n              </div>";
    }
  }
  if (pricing.tierQtyLabelExplanationHtml != null) {
    html +=
      ...
      pricing.tierQtyLabelExplanationHtml +
      "</div>";
  }
  html += "</div>";
  ...
  if (pricing.tiers != null) {
    ...
    eltSlider_1.oninput = function () {
      var qtyValue = PricingHelper.convertSliderValueToQtyValue(
        parseInt(eltSlider_1.value),
        pricingVM_1.tierVMs
      );
      var strQtyValue = qtyValue.toLocaleString("en-US");
      eltTierCostDetail_1.innerHTML = PricingHelper.computeMeteredPlanCost(
        pricing.tiers,
        qtyValue
      ).strEstimatedCost;
